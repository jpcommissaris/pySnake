...
class Board:
    ...
    def __init__(self):
        ...
        self.colorDic = {
            ...
            1: RED,
            1.1: RED,
        }  # 0 is the board, 1 is the head, -1 is a pellet
        # Scores
        self.p1 = 0
        self.p2 = 0
    ...
    def set(self):
        ...
        self.drawSnake(self.s2, int(self.s2.size - 0.1))
    ...
    def setDirection(self):
        ...
        if keys[pygame.K_DOWN] or keys[pygame.K_SEMICOLON]:
            if self.s2.notAllowed != "DOWN":
                self.s2.direction = "DOWN"
        if keys[pygame.K_UP] or keys[pygame.K_p]:
    ...
    def drawBoard(self, win):
        ...
        for r in range(len(self.board)):
            for c in range(len(self.board[r])):
                ...
                pygame.draw.rect(
                    win, color, (c * self.SIZE, r * self.SIZE, self.SIZE, self.SIZE)
                )
        # handles diretions
        self.s1.setAllowed()
        self.s2.setAllowed()
    ...
    def move(self, snake, r, c):
        if (
            snake.headR >= self.wSize - 1
            or snake.headR <= 0
            or snake.headC >= self.lSize - 1
            or snake.headC <= 0
        ):  # borders
            self.game_over = True
        else:
            # checks collisions
            if (
                self.board[snake.headR + r][snake.headC + c] == -1
    ...
    class Snake:
        def __init__(self, r, c, p):
            # position of head
            self.size = 0  # length of snake
            self.headR = r  # row of snakes head
            self.headC = c  # column of snakes head
            self.tailR = r - 5
            self.tailC = c
            self.direction = "DOWN"
            self.notAllowed = "UP"  # direction that cant be pressed
            self.player = p
            self.setSize()
        ...
        def setSize(self):
            # avoid conflicts with coloring, make the size different for each snake
            if self.player == 1:
                self.size = 7
            else:
                self.size = 7.1
